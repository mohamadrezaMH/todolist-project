...
class ToDoListCLI:
    ...
    def create_project(self):
        ...
        try:
            ...
            print(f"✅ پروژه '{project.name}' با موفقیت ایجاد شد. شناسه: {project.id}")
    ...
    def list_projects(self):
        ...
        for project in projects:
            tasks_count = len(self.task_service.get_tasks_by_project(project.id))
    ...
    def list_tasks(self):
        ...
        try:
            ...
            for task in tasks:
                deadline_str = task.deadline.strftime("%Y-%m-%d %H:%M") if task.deadline else "بدون ددلاین"
                status_icon = "🔴" if task.status == "todo" else "🟡" if task.status == "doing" else "🟢"
                print(f"{status_icon} {task.id}: {task.title} | وضعیت: {task.status} | ددلاین: {deadline_str}")
    ...
    def show_project_stats(self):
        ...
        try:
            ...
            for status, count in stats["status_count"].items():
                status_fa = {"todo": "در انتظار", "doing": "در حال انجام", "done": "انجام شده"}
